...
#pragma pack(push, 1)
    ...
    struct DLLEXPORT M8FiringData
    {
      ...
      std::uint32_t returns_distances[M8_NUM_RETURNS][M8_NUM_LASERS];   // 10 mm resolution.
    ...
    struct DLLEXPORT M8DataPacket
    {
      ...
      std::uint16_t version;     // API version number.  Current number is 4 which sends distance as units of 10 micro-meters, <4 is 10cm

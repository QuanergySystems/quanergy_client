...
namespace quanergy
{
	namespace client
	{
		  ...
		  , degrees_per_cloud_(360.0)
		{
			...
			for (uint32_t i = 0; i < M8_NUM_LASERS; ++i, ++angle_in_radians)
			{
		...
		void DataPacketParserM8::setCloudSizeLimits(int32_t szmin, int32_t szmax)
		{
			if (szmin > 0)
			...
			for (int i = 0; i < M8_FIRING_PER_PKT; ++i)
			{
				...
				for (int j = 0; j < M8_NUM_LASERS; j++)
				{
					...
					if (return_selection_ == ReturnSelection::ALL)
					{
						...
						uint32_t d = data.returns_distances[0][j];
						if (d != 0)
						{
